...
- `@dataclass LambdaOutput(lambda_select, lambda_jump, jump_drift, e_loss, ci_low, ci_high)` (:30)
- `category_base_rate(category, dispute_counts=None, counts=None) -> dict` (:48) — delegates to `data.base_rates`
- `fit_hazard(labeled_rows) -> (model, metrics)` (:68) — class-weighted `LogisticRegression`, reports Brier; needs ≥30 rows spanning both classes
- `estimate_lambda(market_conditionid, features, *, dispute_counts=None, model=None, kappa_loss=DEFAULT_KAPPA_LOSS) -> LambdaOutput` (:117)
- consts `SAFE_FEATURES = (category_base_rate, market_size, proposer_reliability, latency_anomaly)` (:40), `DEFAULT_KAPPA_LOSS=0.76` (:45)
...
- `class LoadedHazard(coef, intercept, feature_order, offset=0.0)` with `predict_proba(X)` (:90) — no sklearn/pickle at load
- `save_hazard_model` (:101) / `load_hazard_model` (:113) → `.data_cache/hazard_model.json`
- training set builders: `build_training_rows(*, control_per_category=200)` (:292), `build_matched_training_rows(...)` (:356, CEM-matched controls), `_cem_match(...)` (:319), `load_controls_from_indexer(n, *, graphql_url=None)` (:229)
- `train_and_cache(*, matched=False, graphql_url=None, path=...)` (:404), `fit_and_save(...)` (:448, held-out Brier+AUC + prior-correction offset)
- **Entry point:** `python -m estimators.hazard [--matched] [--graphql-url] [--path]` (:487)
...
- `@dataclass QuoteParams(gamma=0.5, k=5.0, kappa=1.0, min_horizon=0.02, boundary_floor=0.002, base_inventory_cap=100.0)` (:75)
...
- `@dataclass MarketState(cid, token_id, category, arm, end_date_ts, inventory=0, cash=0, order_ids, micro, lam=None, sigma_prior=0.15, defensive=False, sim_reward_score=0, n_exits=0)` (:109) — `arm ∈ {lambda_on, lambda_off}`
...
- `class PaperClob(token_ids, *, seed=7)` (:91) — fully synthetic, deterministic; `SyntheticBook` (:51, seeded latent logit random walk + Poisson taker prints), `SimOrder` (:35)
...
- `@dataclass AblationResult(arm, lambda_star, n_disputes, n_controls, pnl_net_of_rewards, sharpe, avoided_loss, forgone_rewards)` (:41)
- `power_calc(markets_quoted, dispute_rate, resting_fraction)` (:52)
- `load_disputes(graphql_url)` (:63) — DATA_SOURCE switch (hf parquet vs graphql)
- `_replay_market(...)` (:113, calls `execution.loop.should_exit`), `run_replay(graphql_url, lambda_star_grid, *, control_ratio=3, fill_limit=5000, ...)` (:187)
- **Entry point:** `python -m forwardtest.replay_ablation` (:300) — grid `[0.0005,0.001,0.002,0.005,0.01]`
...
- **`hf.py`** — DuckDB connection + source switch + path resolver + verified column registry. `table_path` (:106), `connect()` (:123, cached, httpfs), `with_retry` (:148), `query`/`query_df` (:170/:175), `live_columns` (:180, drift guard). Consts `HF_DATASET`, `DATA_SOURCE`, `CACHE_DIR`, `TABLE_LAYOUT`, `COLUMNS`.
- **`disputes.py`** — OOv2 dispute labels **without Docker**. `derive_condition_id(adapter, ancillary)` (:161) = keccak(adapter‖keccak(ancillary)‖2); `load_disputes()` (:714, released parquet → indexer → RPC fallback), `dispute_counts_by_category()` (:768, the λ numerator), `fetch_oov2_disputes` (:177), `resolve_indexer` (:531). Consts `OOV2`, `NEGRISK`, `ADAPTER_OF`, `RELEASE_PARQUET`. **Entry:** `python -m data.disputes` (:795).
- **`negrisk_map.py`** — recovers UMA↔tradeable conditionId for NegRisk from `QuestionPrepared` logs. `build_negrisk_map` (:85, canary-validated), `load_negrisk_map()` (:210), `derive_negrisk_cid` (:58). **Entry** (:150).
...
- `run_recon(graphql_url, rpc_url="", confirmation_depth=128, *, chain_head_ts=None, log=print)` (:98)
- **Entry point:** `python -m recon.check` (:170). **Imported by:** `webapp` services (recon-live).
