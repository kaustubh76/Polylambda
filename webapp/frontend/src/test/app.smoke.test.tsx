...
// the app fires a lot of fetches on mount (overview, live status, testnet polls). Reject them all —
...
describe('<App/> smoke', () => {
  it('mounts the shell (providers, header, eager sections) without throwing', () => {
    ...
    // an eager, above-the-fold section header is present
    expect(screen.getByText(/Trade against the PolyLambda engine/i)).toBeInTheDocument()
  })
})
