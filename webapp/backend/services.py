...
def testnet_ablation() -> dict:
    ...
    from execution.testnet_keeper import latest_session_path, session_date_from_path
    ...
    out["available"] = True
    out["is_live"] = is_live
    out["session_date"] = session_date_from_path(path)
    return out
...
def testnet_session(limit: int = 60) -> dict:
    ...
    return {"available": True, "is_live": is_live, "session_date": session_date_from_path(path),
            "session_log_path": path, "n_records": len(recs),
